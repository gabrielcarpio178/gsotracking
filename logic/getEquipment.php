...
if(isset($_POST['search'])){
    $search = $_POST['search'];
    if($search==='all'){
        $sql = "SELECT u.`fullname`, u.`usercode`, p.`purchase_request_code`, p.`datetime`, l.`item_name`, l.`quantity`, l.`status`, l.`id` AS purchase_request_id, l.`doingMaintenance`, p.`id`, l.`maintance` AS maintance_durition, l.`request_maintenance`, l.`isDisabled` FROM purchase_request_list AS l JOIN purchase_request AS p ON p.purchase_request_code = l.purchase_request_code JOIN users AS u ON l.requester_code = u.usercode WHERE l.status != 'pending' ORDER BY l.`id` DESC";
    }else{
